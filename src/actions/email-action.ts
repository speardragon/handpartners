...
export async function sendJudgingEmails(
...
): Promise<SendJudgingEmailsResult> {
  const supabase = await createClient();

  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) handleError(new Error("인증되지 않은 사용자입니다."));
  ...
  if (roundError || !judgingRound)
    handleError(roundError ?? new Error("심사 라운드를 찾을 수 없습니다."));

  if (judgingRound.status !== "IN_PROGRESS")
    handleError(
      new Error("심사가 진행 중일 때만 이메일을 발송할 수 있습니다.")
    );
  ...
  if (judgesError) handleError(judgesError);
...
export async function getJudgeEmailCount(
  judgingRoundId: string
): Promise<{ total: number; withEmail: number }> {
  const supabase = await createClient();
